...
object DBTeapotRepository extends TeapotRepository {
  ...
  override def createTeapot(teapot: Teapot): Future[Unit] =
    DB.futureLocalTx(implicit session => Teapot.create(teapot).map(_ => ()))

  override def getTeapot(id: Long): Future[Option[Teapot]] =
    DB.futureLocalTx(implicit session => Teapot.findById(id))
}
